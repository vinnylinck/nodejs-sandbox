...
class SandApp {
  ...
  async Run(port, dbUrl, secOpts, tmpOpts) {
    ...
    // open routes
    this.app.use('/bye', express.static('public'));
    ...
    // loading http routes
    this.app.use('/', this.#ensureAuth, express.static('resources'));
    ...
    this.app.delete('/tools/access/sessions', this.#ensureAuth, routes.Delete_ToolsAccess_Sessions);
    this.app.delete('/tools/access/data', this.#ensureAuth, routes.Delete_ToolsAccess_Data);
    ...
    this.app.get('/tools/events/stream', this.#ensureAuth, ...routes.Stream_ToolsEvents);
